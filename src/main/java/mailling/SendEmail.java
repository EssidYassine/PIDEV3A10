...
public class SendEmail {
    ...
    public static void sendConfirmationEmail(String toEmail, User user, Reservation reservation)
        ...
        try {
            message.setFrom(new InternetAddress(FROM_EMAIL, "Eventus", "UTF-8"));
            message.addRecipient(Message.RecipientType.TO, new InternetAddress(toEmail));
            message.setSubject("Confirmation de réservation #" + reservation.getReservationId());

            message.setContent(
                    buildEmailTemplate(user, reservation),
            ...
            throw new MessagingException("Adresse email invalide: " + toEmail);
        } catch (IllegalStateException e) {
            throw new MessagingException("Erreur de configuration SMTP");
        }
    }

    private static String buildEmailTemplate(User user, Reservation reservation) {
        return "<!DOCTYPE html>"
                + "<html>"
                + "<head>"
                + "<style>"
                ...
                + "   .header { color: #2c3e50; font-size: 24px; border-bottom: 2px solid #eee; padding-bottom: 10px; }"
                ...
                + "           <div class='details'>"
                + "               <p><strong>Date de votre réservation :</strong> " + formatDate(reservation) + "</p>"
                + "               <p><strong>Participants :</strong> " + reservation.getNbreInvites() + "</p>"
                + "           </div>"
                + "           <p>Cordialement,<br>L'équipe de Eventus</p>"
                + "       </div>"
                + "   </div>"
                + "</body>"
                + "</html>";
    ...
    private static String formatDate(Reservation reservation) {
        return reservation.getDateReservation().toLocalDateTime()
                .format(DateTimeFormatter.ofPattern("EEEE dd MMMM yyyy 'à' HH'h'mm"));
    }
}